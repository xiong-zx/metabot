...
| `WIKI_AUTO_SYNC_WATCH_ROOT` | 来源根目录 | `WIKI_SYNC_SOURCE_ROOT` 的兼容别名；显式配置时必须一致 |
...
- `WIKI_SYNC_SOURCE_ROOT=/imac` 会把 Memory `/imac` 中的内容直接投影到配置的
  Wiki 根节点下，不会再创建一层同名 `imac`。全量对账和增量事件使用同一个来源边界。

多台主机共享同一个 Space 时，每台主机必须使用不同的根节点和状态目录，使目标绑定和映射彼此独立。
默认消费者名称由 Space 和根节点 token 派生，因此共享同一个 Memory Core 的主机不会共用游标。

不要把已有映射的状态目录改指向另一个 Space、Wiki 根节点或 Memory 来源根目录。
目标绑定不可变；任一目标变化时都应使用新的空 `WIKI_SYNC_STATE_DIR`。
