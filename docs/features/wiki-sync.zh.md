...
| `WIKI_SYNC_DELETE_STALE_DOCS` | `true` | 源 MetaMemory 文档消失时，删除已映射的飞书知识库文档节点 |
...
folder 和 document ID 会保留。文档路径变更后，下一次同步会创建新的知识库映射；`WIKI_SYNC_DELETE_STALE_DOCS=true` 时，会请求飞书删除上一个已映射的旧文档节点。MetaMemory 中消失的文档映射，也会在全量同步时通过 service app 身份删除对应飞书知识库节点。若 service app 缺少删除权限，同步会记录错误并保留 mapping，等权限修好后下次继续重试。
...
- `wiki:node:create` — 创建 / 删除知识库节点；全量同步清理 stale 节点需要该权限
...
- 全量同步会清理 stale 文档映射；`WIKI_SYNC_DELETE_STALE_DOCS=true` 时，还会通过 service app 删除对应飞书知识库节点，不需要 user OAuth。
- 手动 `/sync` 始终可用
