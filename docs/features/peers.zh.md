...
=== "环境变量 (.env)"
    ...
    - `METABOT_PEER_SOURCE_BOTS` — 可选；按位置指定每个 peer 使用的本机出站 Bot
...
=== "bots.json"
    ...
    - `auth.sourceBot` — 作为签名出站任务身份的明确本机 Bot
...
源与目标 Bot、可选 RulesPack issuer、chat ID、request ID、body SHA-256、签发/过期时间和一次性 nonce。
默认有效期为 30 秒。重放、正文篡改、错误路由、过期、未知发行方和已撤销 key
都会 fail closed，并返回稳定错误码。

`auth.sourceBot` 与 RulesPack envelope issuer 是两个不同身份。前者必须是发送
Bridge 上的本机 Bot，并绑定 peer task；后者仍是 RulesPack transport identity，
会作为独立 capability claim 被签名。当本机有多个 Bot 能编译出站 RulesPack 时，
应显式设置 `sourceBot`，避免 Bridge 猜测由哪个 operator 负责。
