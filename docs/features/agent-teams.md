...
- Implemented atomic restart execution: restart requests now progress through `scheduled/forced -> restarting -> healthy/failed`, one `requestId` can claim only one PM2 attempt, the old bridge submits only one same-runtime `pm2 restart --update-env`, and the new bridge performs health checks plus `pm2 save`. Runtime/worktree switching is refused inside the MetaBot process tree and must use external `metabot deploy-runtime`.
