...
!!! note "回复上下文需要消息读取权限"
    **`im:message:readonly`** 权限让 MetaBot 能按飞书/Lark 的“回复”操作读取用户明确选中的原消息。在仅 @ 回复的群聊中，未 @ 的消息或附件本身不会进入模型上下文；请回复该消息并 @ 指定机器人。支持引用文本、富文本消息、图片、文件和交互卡片；同一聊天内的卡片无论作者是谁都会保留可见文字，跨聊天引用会被拒绝，提取文字最多 16,000 个字符。之后只做普通 @，不会附带其他未被引用的缓存媒体。使用 `feishuDomain: "lark"` 的 Lark 应用也要授予同一权限。
