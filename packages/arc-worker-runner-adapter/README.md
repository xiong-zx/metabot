...
given an ordinary authorized user/PM principal. The production
`pm/arc-service/local:arc-service` machine credential is intentionally limited
to dispatch and observation, so cancellation with that credential fails
closed instead of granting it abort authority. A
...
identity to dispatch plus same-scope list/status; it cannot request all scopes,
abort, or redirect work. Direct chat-scoped Worker tools cannot see or abort
