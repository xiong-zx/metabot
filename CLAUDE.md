...
- 2026-07-22 MEM-011 CLI 测试隔离：`packages/cli/vitest.config.ts` 必须显式 `fileParallelism:false`（保留 `pool:'forks'` + `singleFork:true`），因为 CLI 测试会替换/修改共享 `process.env`；发布前 canonical `npm test` 需要连续多次稳定通过，不能只用 isolated CLI run 代替。
