...
interface AuthenticatedPeerForwardInput {
  ...
  body: { botName: string; chatId: string; prompt: string; sendCards?: boolean; rulesPackDispatch?: RulesPackDispatchEnvelopeV1 };
}
...
export async function forwardAuthenticatedPeerTask(input: AuthenticatedPeerForwardInput): Promise<object> {
  if (input.body.rulesPackDispatch || input.peerBot.engine !== 'codex') {
  ...
  try {
    return await input.peerManager.forwardTask(input.peer, { ...input.body, rulesPackDispatch: envelope });
  } catch (error) {
