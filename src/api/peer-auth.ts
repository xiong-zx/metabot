...
export interface PeerCapabilityClaims {
  ...
  path: string;
  sourceBot: string;
  /** RulesPack transport identity, signed separately from the local source Bot. */
...
function isClaims(value: unknown): value is PeerCapabilityClaims {
  ...
  return claims.v === PEER_CAPABILITY_VERSION
    ...
    && (claims.rulesPackIssuer === undefined
      || (typeof claims.rulesPackIssuer === 'string' && claims.rulesPackIssuer.length > 0))
