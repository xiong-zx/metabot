...
export class PeerManager {
  ...
  constructor(
    ...
    for (const config of configs) {
      this.assertConfiguredSourceBot(config);
  ...
  private assertConfiguredSourceBot(peer: PeerConfig): void {
    const configured = peer.auth?.sourceBot?.trim();
    if (configured && !this.localBots.some((bot) => bot.name === configured)) {
      throw new Error(`peer_auth_source_bot_not_local:${configured}`);
    }
  }

  private resolveSourceBot(peer: PeerConfig, requested: unknown): string {
    const configured = peer.auth?.sourceBot?.trim();
    if (configured) {
      if (!this.localBots.some((bot) => bot.name === configured)) {
        throw new Error(`peer_auth_source_bot_not_local:${configured}`);
      }
      if (typeof requested === 'string' && requested.trim() && requested !== configured) {
        throw new Error(`peer_auth_source_bot_mismatch:${requested}`);
      }
      return configured;
    }
  ...
  private capabilityAuthorization(
    ...
    const claims: PeerCapabilityClaims = {
      ...
      ...(binding.rulesPackIssuer ? { rulesPackIssuer: binding.rulesPackIssuer } : {}),
  ...
  verifyInboundPeerRequest(input: {
    ...
    if (claims.method === 'POST') {
      ...
      const rulesPackDispatch = body.rulesPackDispatch;
      const bodyRulesPackIssuer = rulesPackDispatch && typeof rulesPackDispatch === 'object'
        && typeof (rulesPackDispatch as { issuer?: unknown }).issuer === 'string'
        ? (rulesPackDispatch as { issuer: string }).issuer
        : undefined;
      if (typeof body.botName !== 'string' || !body.botName || body.botName.includes('/')
        ...
        || bodyRulesPackIssuer !== claims.rulesPackIssuer
   ...
   * exact outbound hop. Scoped peer auth signs the transport issuer separately
   * from its explicit local source Bot; Agent Bus traffic uses the
   * server-derived `/api/whoami` botName.
   */
  private async assertRulesPackTransportIssuer(peer: PeerConfig, body: object): Promise<void> {
    ...
    if (peer.auth) {
      const configuredSourceBot = peer.auth.sourceBot?.trim();
      if (!configuredSourceBot) {
        this.logger.warn(
          { peerName: peer.name, issuer, authMode: 'peer-capability' },
          'RulesPack peer dispatch has no explicit local source Bot',
        );
        throw new Error('RulesPack peer dispatch requires auth.sourceBot');
      }
      const sourceBot = typeof (body as { sourceBot?: unknown }).sourceBot === 'string'
        ? (body as { sourceBot: string }).sourceBot.trim()
        : configuredSourceBot;
      if (sourceBot !== configuredSourceBot || !this.localBots.some((bot) => bot.name === sourceBot)) {
        this.logger.warn(
          { peerName: peer.name, issuer, sourceBot, configuredSourceBot, authMode: 'peer-capability' },
          'RulesPack peer dispatch source Bot does not match local configuration',
        );
        throw new Error('RulesPack peer dispatch source Bot must match auth.sourceBot');
      }
      this.logger.info(
        { peerName: peer.name, issuer, sourceBot, authMode: 'peer-capability' },
        'RulesPack issuer and local source Bot bound to scoped peer capability',
      );
      return;
    }
  ...
  async forwardTask(peer: PeerConfig, body: object): Promise<object> {
    ...
    const requestId = /^[A-Za-z0-9._:-]{1,128}$/.test(requestedId)
      ? requestedId
      : crypto.randomUUID();
    const sourceBot = this.resolveSourceBot(effectivePeer, input.sourceBot);
    const targetBot = typeof input.botName === 'string' ? input.botName : '';
    const chatId = typeof input.chatId === 'string' ? input.chatId : '';
    const rulesPackDispatch = input.rulesPackDispatch;
    const rulesPackIssuer = rulesPackDispatch && typeof rulesPackDispatch === 'object'
      && typeof (rulesPackDispatch as { issuer?: unknown }).issuer === 'string'
      ? (rulesPackDispatch as { issuer: string }).issuer.trim()
      : undefined;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const attemptHeaders = {
        ...
        ...this.authorizationHeaders(state, 'POST', '/api/talk', rawBody, {
          sourceBot,
          ...(rulesPackIssuer ? { rulesPackIssuer } : {}),
          targetBot,
          chatId,
          taskId: requestId,
        }),
