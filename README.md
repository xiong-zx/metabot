...
| **Skill Hub** | 跨实例技能共享注册中心。`metabot bot-skills` 发布、发现、安装技能，FTS5 全文搜索 |
...
安装器将 `metabot` 放到 `~/.local/bin/`，安装后立即可用。`metabot` 是**唯一的 CLI 入口**，三类命令：(1) bridge 进程控制（`update` / `start` / `stop` / `restart` / `logs` / `status`）；(2) bridge 守护进程 API（`bots` / `talk` / `schedule` / `peers` / `stats` / `voice` / `bot-skills` / `health`，curl 本地 `localhost:9100`）；(3) 其余（`t5t` / `agents` / `memory` / `skills`）转发给本仓库 monorepo 内 `packages/cli/bin/metabot` 提供的 metabot-core 功能 CLI。旧的 `mb` 命令降级为兼容壳，转发到 `metabot`；`mm`/`mh` CLI 与 `metamemory`/`skill-hub` skill bundle 已下线。
...
# 每个 Bot 的 Skill Hub（bridge 本地）
metabot bot-skills list               # 列出所有技能
metabot bot-skills search <关键词>     # 搜索技能
metabot bot-skills publish <bot> <skill> # 发布 Bot 的技能
metabot bot-skills install <skill> <bot> # 安装技能到 Bot
