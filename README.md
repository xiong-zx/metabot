...
> **模型切换**：每个会话可独立设置模型。默认 `claude-opus-4-8`（1M 上下文默认开启）。对于 Opus 4.7/4.6 和 Sonnet 4.6，在模型名后加 `[1m]` 可启用 1M 上下文窗口，例如 `/model claude-opus-4-7[1m]`。OAuth/Pro-Max 登录用户 SDK 会丢弃 beta flag，`[1m]` 后缀是这些老模型上唯一可靠的 1M 开启方式。
