...
安装器将 `metabot`、`mb` 放到 `~/.local/bin/`，安装后立即可用。`metabot` 是统一调度器：保留 bridge 进程控制子命令（`update` / `start` / `stop` / `restart` / `logs` / `status`），其余（`t5t` / `agents` / `memory` / `skills`）一律转发给 [metabot-core](https://gitlab.xvirobotics.com/xvirobotics/metabot-core) 功能 CLI（自动检测同级 `../metabot-workspace/metabot-core`）。Phase 4 后旧的 `mm`/`mh` CLI 与 `metamemory`/`skill-hub` skill bundle 已下线。
